...
const convertHrefToHtml = (href) => {
  return href.replace(/\.json$/, ".html").replace(/\.yml$/, ".html");
};

const refToLink = (ref, label) => {
  let isHttpRef = ref.match(/^http(s)?:/);
  let refParts = ref.split("/");
  let filename = refParts[refParts.length - 1];
  filename = filename.substr(0, filename.length - 5);
  filename = convertHrefToHtml(ref);

  if (isHttpRef || !label) {
    let href = ref;
    if (!isHttpRef) {
      href = convertHrefToHtml(href);
    }

    return `<a href="${href}">${typeToLinkLabel(filename, ref)}</a>`;
  }

  // if we have a definition convert it to a html anchor to the property
  filenameParts = filename.split("#");

  if (
    filenameParts.length === 2 &&
    filenameParts[1].match(/^\/(definitions|\$defs)/)
  ) {
    filename =
      filenameParts[0] +
      "#" +
      filenameParts[1].replace(/(definitions|\$defs)/, "");
  }
...
const oneOfRow = (name, property) => {
  ...
  for (const key in property.oneOf) {
    ...
    html +=
      "<td>" +
      jsonSchemaType(oneOfItem.type ? oneOfItem : property.type) +
      "</td>" +
      "</tr>";
...
const oneOfArrayRow = (oneOf) => {
  ...
  oneOf.forEach((one) => {
    ...
    html += "<th>" + jsonSchemaType(one) + "</th>" + "</tr>";
...
Handlebars.registerHelper("propertyTypeRow", function (property) {
  ...
  if (property.oneOf != undefined) {
    ...
    const typeLabel = jsonSchemaType(property);
    if (typeLabel) {
